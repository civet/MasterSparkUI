			...
			switch(state)
			{
				case Button.STATE_DOWN:
					if( _downScale9Grid ) {
						g = _face.graphics;
						g.clear();
						fill9Grid( g, _downTexture, getRectangle(0,0,w,h), _downScale9Grid, false);
					}
					else {
						g = _face.graphics;
						g.clear();
						fillBitmap(g, _downTexture, getRectangle(0,0,w,h) );
					}					
					_face.filters = _faceFilters0;
					break;
				
				case Button.STATE_DISABLED:
					if( _normalScale9Grid ) {
						g = _face.graphics;
						g.clear();
						fill9Grid( g, _normalTexture, getRectangle(0,0,w,h), _normalScale9Grid, false);
					}
					else {
						g = _face.graphics;
						g.clear();
						fillBitmap(g, _normalTexture, getRectangle(0,0,w,h) );
					}
